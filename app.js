...
/** ===== Barra de categorias (botões) =====
  onChange(cat) é chamado quando muda; retorna um objeto com get/set
*/
function bindCategories(onChange) {
  const bar = byId('categoriesBar');
  if (!bar) return { get: () => 'Todos', set: () => {} };

  const buttons = $$('.cat-btn', bar);
  let current = 'Todos';

  // Define estado inicial
  const activeInitial = buttons.find(b => b.classList.contains('active')) || buttons[0];
  if (activeInitial) {
    current = activeInitial.dataset.category;
    buttons.forEach(b => b.setAttribute('aria-pressed', String(b === activeInitial)));
  }
  ...
  return {
    get: () => current,
    set: (name) => {
      const btn = buttons.find(b => b.dataset.category === name);
